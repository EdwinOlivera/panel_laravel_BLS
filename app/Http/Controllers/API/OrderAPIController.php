...
class OrderAPIController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($payment['payment']) && $payment['payment']['method']) {
            ...
            } else if ($payment['payment']['method'] == "Tarjeta de Crédito") {
                return $this->paymentPixelPay($request);
    ...
    private function stripPayment(Request $request)
    {
        ...
        try {
            ...
            if ($stripeToken->created > 0) {
                ...
                if(isset($productOrder['extra_amount']) && $productOrder['extra_amount']){
                    $amount += $productOrder['extra_amount'];
                }
    ...
    private function cashPayment(Request $request)
    {
        ...
        try {
            $order = $this->orderRepository->create(
                $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint', 'orden_programada', 'fecha_entrega', 'hora_entrega', 'descripcion','order_number_fac','extra_amount')
            );
            ...
            if(isset($input['extra_amount']) && $input['extra_amount']){
                $amount += $input['extra_amount'];
            }
            $amount += $order->delivery_fee;
            $amountWithTax = $amount + ($amount * $order->tax / 100);
            $payment = $this->paymentRepository->create([
    ...
    private function paymentPixelPay(Request $request)
    {
        $input = $request->all();
        $amount = 0;
        try {
